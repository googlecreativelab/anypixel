 ...
 * 	- Added sinusoidal movement to vertex shader
 * 	- Added uv coordinate scrolling to fragment shader
 */
...
'    normal *= .5 * w;',
'',
'		 float phaseOffset = sin(20.0 * phase + uv.x * 20.0) * 0.3;',
...
'		 float s = 1.0;',
'    vec4 c = vColor;',
'    if( useMap == 1. ) c *= texture2D(map, mod(vUV * s - offsetTime, 1.0));',
'		 if (vUV.x > transition) c = vec4(0, 0, 0, 1);',
'    gl_FragColor = c;',
'',   
'}' ];
	...
	delete parameters.offsetTime;
	delete parameters.phase;
	delete parameters.transition;
